...
public class Drone_design_0 extends PApplet {
...
public void setup() {
  ...
  bt2.posRect((17*width/20)+batWid,PApplet.parseInt(1.5f*batWid));
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
public void mouseWheel(MouseEvent event){  mw = event.getCount();  }

public void movement(Thrust t1, Thrust t2)
{
...
public void movement(Thrust t1, Thrust t2,Thrust t3,Thrust t4,int th)
{
  t1.stThrst(th);
  t2.stThrst(th);
  t3.stThrst(th);
...
class Shape
{
  int pos_x, pos_y;
  int i, j;
  public void posRect(int i, int j)
  {
    pos_x = (width/20)+i;
    pos_y = (height/20)+j;
  }
  public void drawRect(int len, int br, int curve, int thrst_val, String name)
  {
    fill(0);
    stroke(0);
    ...
    text(thrst_val, pos_x+(rectSize/2), pos_y+rectCol+(rectSize/1.5f));
  }
  public void drawRect(int len, int br)
  {
    stroke(0);
    rect(pos_x, pos_y, len, br, 7);
  }
  public void btnClick(int x_cord, int y_cord, int x_range,int y_range,Thrust t,int inr)
  {
    fill(0);
  ...
  public boolean btnClick(int x_cord, int y_cord, int range,char dir)
  {
...
class ControlButtons extends Shape
{
  ...
  public char getDir(){return dir;}                                  ////////Returns the char assigned to the Button/////////////////
  
  public void drawShape(int len, int br)                //////////Draws the Button from the Base Class Shape////////////
  {   
...
class Thrust extends Shape
{
  ...
  public int getThrust(){return thrst;}
  public void setThrust(int thrst)
  {
  ...
  public void drawShape(int len, int br, int thrst,String name)
  {  
    fill(0);
    circle(super.pos_x+(rectSize/2), super.pos_y+rectCol+(rectSize/1.5f), rectSize);  //Thrust display circle
    fill(255);
    super.drawRect(len, br, 7, thrst,name);
    dispThrust(thrst);
    dispThrustControl();
  }
  public void dispThrust(int thrst)
  {
    fill(0,0,255);
    noStroke();
    float i;
    for(i=100; i<(thrst/5)-105;i+=5)
      rect(super.pos_x+2,super.pos_y+(1.5f*rectCol)-i-7,.96f*rectSize,(rectCol/20)-5.75f,1);
    stroke(50);
  }
  public void dispThrustControl()
  {
    thrst1 = thrst;
    ...
    fill(255);
    text("+",inr_x+(inr_size/2),inr_y+(inr_size/2.5f));  //Thrust display circle
    super.btnClick(inr_x,PApplet.parseInt(inr_y+(1.25f*inr_size)),inr_size,inr_size,this,-thrt_pow);
    rect(inr_x, inr_y+1.25f*inr_size, inr_size,inr_size);  //Thrust display circle
    fill(255);
    text("-",inr_x+(inr_size/2),inr_y+1.6f*inr_size);  //Thrust display circle
...
class Battery extends Shape
{
  ...
  public void drawBat(int len, int br, String name)                //////////Draws the Button from the Base Class Shape////////////
  {    
    fill(0);
    super.drawRect(len, br);                            //////////Calls the drawRect() method of base class/////////////
    fill(255);
    textAlign(LEFT,CENTER);
    text(name, super.pos_x-1.75f*batWid, super.pos_y,batWid*2,batWid);
    dispVolt();
  }
  public void dispVolt()
  {
    fill(0);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "Drone_design_0" };
    if (passedArgs != null) {
